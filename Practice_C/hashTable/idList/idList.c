...
/* we'll actually malloc more space than this */
IDList IDListCreate(int n, int unsortedIdList[])
{
    ...
    for(i = 0; i < n; i++) {
        ...
        for(probe = unsortedIdList[i] % list->size; list->ids[probe] != NULL_ID;  probe = (probe + 1) % list->size);
...
int IDListContains(IDList list, int id)
{
    ...
    for(probe = id % size; list->ids[probe] != NULL_ID; probe = (probe + 1) % size) {
